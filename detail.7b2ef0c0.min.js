...
$(document).ready(function() {
    ...
    $('#exchange-markets-other').dataTable({
        "bFilter": false,
        "bInfo": false,
        "bLengthChange ": false,
        "bPaginate": false,
...
CurrencyDetailGraph.prototype.fetchAndLoad = function(callback, start, end) {
    ...
    $.ajax({
        url: this.apiDomain + "/particl.json?" + timeParams,
        type: "GET",
        dataType: "json",
        error: function() {
            that.hideLoading();
...
function loadFCAS() {
    ...
    script.onload = function() {
        var yourApiKey = "6e9c2808-0c48-41d8-81f1-5363d5ceb1af"
        var flipsideJs = new Flipside(yourApiKey);
        var coinName = $("#metadata").data("name");
        var symbol = $("#metadata").data("symbol");
        var coinId = $("#metadata").data("cryptocurrencyid");
        var seriesName = symbol + " FCAS";
        var seriesTitle = coinName + " FCAS";
        var rankEnabled = (coinId == 1776 || coinId == 2694) ? false : true;
        flipsideJs.spectrum("js-fcas-spectrum", {
            asset: {
                asset_id: coinId
            },
            rank: {
        ...
        flipsideJs.chart("js-fcas-chart", {
            title: seriesTitle,
            series: [{
                name: seriesName,
                asset_id: coinId,
    ...
    script.src = "https://d3sek7b10w79kp.cloudfront.net/flipside-v1.10.0.js";
    document.head.appendChild(script);
}

function loadRatingsJS() {
    if ($('#js-fcas-spectrum').is(':empty')) {
        loadFCAS();
    }
}
$(function() {
    ...
    if (!hash) {
        hash = "#charts"
    } else if (hash !== "#charts" && hash !== "#social" && hash !== "#tools" && hash !== "#ratings") {
        hash = "#markets"
    }
    ...
    $('ul.nav a[data-target="' + hash + '"]').tab('show');
    $('.cmc-nav-tab').on('click', function(e) {
    ...
    $('body').on('click touchend', function(event) {
        ...
        if ($target.data('toggle') !== 'tooltip' && $target.parents('.js-exchange-alert-tooltip').length === 0) {
            $('.js-exchange-alert-tooltip').tooltip('hide');
        }
    });
});
Currency.onChange(function(e, currency) {
